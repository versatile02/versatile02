# 👋 Hi, I'm Arhamuddin

🚀 B.Tech CSE (AI & ML) @ JIS University | Aspiring Web Developer  

I am currently building web development skills and working towards becoming a full-stack developer.

---

## 🎯 Current Focus
- 🌐 Learning MERN Stack (React, Node.js, Express, MongoDB)
- 🧠 Strengthening Data Structures and Problem Solving
- 🚀 Building web-based projects and improving backend fundamentals

---

## 🛠️ Tech Stack

**Frontend:**  
HTML, CSS, JavaScript  

**Backend (Learning):**  
Node.js, Express.js  

**Database:**  
MongoDB  

**Tools:**  
Git, GitHub, VS Code  

---

## 🚀 Projects

- ✈️ Flight Booking Web Application  
  A web-based system for booking flights with user authentication and backend integration  

- 🌐 Personal Portfolio Website  
  Responsive portfolio website to showcase projects and skills  

---

## 📜 Publications

- AI-Driven Sentiment Analysis for Product Reviews  
  Published in a symposium (JIS University, 2025)

---

## 📜 Certifications

- Advanced Software Engineering Job Simulation – Forage  
- NPTEL: Design and Analysis of Algorithms  
- NPTEL: Programming in Java  
- Full Stack Development Trainings (MERN, Java Full Stack, Android)

---

## 🔍 Open To

- Web Development Internships  
- Frontend / Backend / Full Stack roles  

---

## 🔗 Connect with Me

- [LinkedIn](https://www.linkedin.com/in/arhamuddin/)
